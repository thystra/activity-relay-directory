...
Activity-Relay Directory 1.1.0 builds on the stable 1.0 service with
operator-controlled relay discovery, independent reachability checks, a V2
public relay API, and an improved human-facing directory. The V1 Protocol and
`/v1/relays` remain compatible with 1.0.
...
- `GET /readyz`
- `GET /v1/status`
- default-off V1 public API at `GET /v1/relays`, V2 public API at
  `GET /v2/relays`, and human-facing directory at `GET /`
...
- V1 Protocol vocabulary and JSON compatibility fixtures
...
- stateless RFC 9421 directory-request verification with a compatibility fixture
- atomic opaque-key replay protection with a concurrency-tested reference
...
- strict authenticated unregister requests without state deletion
...
- local `admin discovery add|remove|import` commands with
  SSRF-resistant actor verification, bounded file import, private provenance, and
  non-mutating inbox diagnostics
- optional background reachability checks that keep relay availability
  information current without changing lifecycle heartbeat status
- a V2 public API at `/v2/relays` with richer discovery and reachability
  information, plus a responsive human-facing directory with Previous and Next
  navigation; `/v1/relays` remains compatible with 1.0
...
The V1 Protocol is stable. The lifecycle routes remain
fail-closed unless
...
Activity-Relay Directory 1.1 also supports local operator discovery without
...
The V2 public API is `GET /v2/relays`. It uses the same default-off
`DIRECTORY_PUBLIC_LISTING_ENABLED` switch as `/v1/relays` and `/`. Enabling the
...
The stable package uses application version `1.1.0` and Debian version
`1.1.0-1`. Package installation is deliberately separate from activation: the
systemd unit is installed disabled and is not started automatically. The
packaged environment binds to loopback, stores SQLite state under
`/var/lib/activity-relay-directory`, and keeps lifecycle, public listing,
...
Normal package upgrade or removal preserves the Directory state and dedicated
service account. Explicit package purge is destructive and removes the state
directory and dedicated service account. Operator-owned
`/etc/activity-relay-directory/config.yml` is not a package conffile and is not
deleted by the custom package-maintenance scripts.

See `debian/README.Debian` and `docs/RELEASING.md` before package installation,
activation, upgrade, downgrade, removal, or purge.
