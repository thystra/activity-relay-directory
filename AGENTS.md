...
Release validation infrastructure is part of the release-safety boundary. Treat the current machine, boot chain, storage topology, and test residue as live authority; a historical verifier report is audit evidence unless it records an irreconstructible human decision or pre-mutation state.

* Never roll back the active root filesystem or active nested ZFS datasets under the running operating system. A `zfs rollback` command may return success while the running kernel still holds incoherent page-cache/writeback state. Restore a boot/root environment from an offline, recovery, initramfs, alternate-boot, or otherwise demonstrably unmounted context.
* Root and boot storage form one bootable state. When root and `/boot` live in separate ZFS pools, define a reusable baseline as a paired rpool+bpool set and preserve the same logical baseline identity across both pools. Omitting the boot pool can leave a newer kernel in `/boot` with no matching module tree in the rolled-back root.
* The EFI System Partition is outside ZFS and needs separate evidence or backup when a baseline is expected to reconstruct the full boot chain. Record its device/UUID and retain a reviewed file manifest or image appropriate to the recovery model.
* ZFS snapshot creation on a mounted dataset is a normal supported operation; live rollback is the dangerous boundary. Cross-pool snapshots are not atomic, so record each pool's snapshot identity and creation time and document the pairing method.
* Before freezing a new reusable host baseline after package maintenance, complete the package transaction, verify `dpkg`/APT state, verify every bootable kernel has its matching initrd and module tree, perform a normal unattended reboot, and re-prove bootloader, service, pool, and test-host health.
* Shared validation hosts are not owned by one project. The clean-room host used for Directory acceptance is also used by WordPress projects for Docker and Plugin Check validation. A reusable host baseline must therefore remain project-neutral: do not preserve candidate packages, containers, volumes, temporary test files, credentials, or project-specific configuration in the baseline.
* A gate that mutates successfully and then fails in its verifier is a partial mutation, not permission to replay the mutation. Inspect current state directly, classify what actually completed, preserve evidence, and write a continuation/recovery gate from that observed state. Do not blindly rerun snapshot creation, upgrade, rollback, package installation, publication, or any other non-idempotent step.
* Verifier privilege is part of the verifier contract. A permission failure while reading a root-owned bootloader/configuration file is not evidence that the file or system is invalid. Run checks with the least privilege that can actually read the authoritative object, and distinguish access failure from content failure.
